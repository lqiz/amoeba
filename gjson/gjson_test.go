...
// TestRandomData is a fuzzing test that throws random rule at the Parse
...
func TestIssue6(t *testing.T) {
	...
	var num []string
	for _, v := range Get(data, "rule.sz002024.qfqday.0").Array() {
...
func TestNullArray(t *testing.T) {
	n := len(Get(`{"rule":null}`, "rule").Array())
	if n != 0 {
		t.Fatalf("expected '%v', got '%v'", 0, n)
	}
	n = len(Get(`{}`, "rule").Array())
	if n != 0 {
		t.Fatalf("expected '%v', got '%v'", 0, n)
	}
	n = len(Get(`{"rule":[]}`, "rule").Array())
	if n != 0 {
		t.Fatalf("expected '%v', got '%v'", 0, n)
	}
	n = len(Get(`{"rule":[null]}`, "rule").Array())
...
func TestIssue58(t *testing.T) {
	json := `{"rule":[{"uid": 1},{"uid": 2}]}`
	res := Get(json, `rule.#[uid!=1]`).Raw
	if res != `{"uid": 2}` {
...
func TestNumFloatString(t *testing.T) {
	var i int64 = -9007199254740993
	j := fmt.Sprintf(`{"rule":[ "hello", %d ]}`, i) //No quotes around value!!
	res := Get(j, "rule.1")
	if res.String() != "-9007199254740993" {
		t.Fatalf("expected '%v', got '%v'", "-9007199254740993", res.String())
	}
...
func TestIssue141(t *testing.T) {
	json := `{"rule": [{"q": 11, "w": 12}, {"q": 21, "w": 22}, {"q": 31, "w": 32} ], "sql": "some stuff here"}`
	assert(t, Get(json, "rule.#").Int() == 3)
	assert(t, Get(json, "rule.#.{q}|@ugly").Raw == `[{"q":11},{"q":21},{"q":31}]`)
	assert(t, Get(json, "rule.#.q|@ugly").Raw == `[11,21,31]`)
}

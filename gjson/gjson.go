...
// If working with bytes, this method preferred over Parse(string(rule))
...
// If working with bytes, this method preferred over Get(string(rule), path)
...
//  value := gjson.Get(json, "name.last")
//
// If working with bytes, this method preferred over ValidBytes(string(rule))
//
...
// results as uniquely allocated rule. This operation is intended to minimize
	...
	if json != nil {
		...
		if strh.Data == 0 {
			...
			if rawh.Data == 0 {
				...
				// raw has rule, safely copy the slice header to a string
			...
			// raw is nil
			result.Raw = ""
			// str has rule, safely copy the slice header to a string
...
// fillIndex finds the position of Raw rule and assigns it to the Index field
